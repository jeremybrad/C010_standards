...
class ProjectRegistry:
    ...
    def __init__(self, registry_path: Optional[str] = None):
        ...
        if registry_path is None:
            ...
            registry_path = workspace / 'SharedData' / 'registry' / 'project_registry.json'
        else:
    ...
    def format_context(self, project: Dict, format_type: str = 'context') -> str:
        ...
        if format_type == 'json':
            ...
            return (
                f"{project['id']} ({project['series_label']}) · "
            ...
            lines = [
                ...
                f"ID: {project['id']} | Series: {project['series_label']} | Status: {project['status']}",
                "",
                f"## Description",
                project['description'],
            ]
...
def main():
    ...
    parser.add_argument(
        '--registry',
        help='Path to project_registry.json (default: ~/SyncedProjects/SharedData/registry/project_registry.json)'
    )
    ...
    # Report not found
    if not_found:
        print(f"\n⚠️  Not found: {', '.join(not_found)}", file=sys.stderr)
        return 1
